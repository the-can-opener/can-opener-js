...
export function decodeSignalValue(
  frame: CanFrame,
  signal: CodecSignal,
...
export function writeSignalValue(
  ...
  const unsigned =
    signal.signed === true ? fromSigned(raw, signal.length) : raw;
  writeRaw(data, signal, unsigned);
}

export function decodeFrameSignals(
  frame: CanFrame,
  signals: Iterable<VehicleSignal>,
): Array<{ signal: VehicleSignal; value: unknown }> {
...
export function isCodecSignal(
  signal: VehicleSignal,
): signal is VehicleSignal & CodecSignal {
  return (
    typeof signal.startBit === "number" && typeof signal.length === "number"
  );
}
...
function writeDataBit(
  ...
  data[byteIndex] =
    bit === 1 ? current | (1 << bitIndex) : current & ~(1 << bitIndex);
}

function motorolaBitPosition(
  startBit: number,
  bitIndexFromMsb: number,
): number {
...
function normalizeNumericValue(value: unknown): number {
  ...
  if (typeof value !== "number" || !Number.isFinite(value)) {
    throw new Error(
      `Signal value must be a finite number or boolean, received ${String(value)}`,
    );
  }
...
function assertSignalFits(data: Uint8Array, signal: CodecSignal): void {
  ...
  const positions =
    signal.byteOrder === "big"
      ? Array.from({ length: signal.length }, (_, index) =>
          motorolaBitPosition(signal.startBit, index),
        )
      : Array.from(
          { length: signal.length },
          (_, index) => signal.startBit + index,
        );
...
function assertIntegerRange(value: number, length: number): void {
  ...
  if (value < 0 || value >= 2 ** length) {
    throw new Error(
      `Encoded raw value ${value} does not fit in ${length} bits`,
    );
  }
}
